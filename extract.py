...
def load_neos(neo_csv_path):
    ...
    with open(neo_csv_path, 'r') as f:
        ...
        for elem in reader:
            ...
            diameter = float('nan') if elem['diameter'] == "" else \
                float(elem['diameter'])
            name = None if elem['name'] == "" else elem['name']
            list_neos.append(NearEarthObject(designation=elem['pdes'],
...
def load_approaches(cad_json_path):
    ...
    with open(cad_json_path, 'r') as f:
        ...
        for elem in data['data']:
            list_ca.append(CloseApproach(designation=elem[0],
                           time=elem[3], distance=float(elem[4]),
